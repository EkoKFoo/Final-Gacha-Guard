...
class _HomePageState extends State<HomePage> {
  ...
  Color _getCategoryColor(String category) {
    final colors = [Color(0xFF7B88FF), Colors.red, Colors.orange, Colors.green, Colors.amber];
  ...
  Widget _buildTransactionItem(TransactionModel tx) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: Colors.grey.withOpacity(0.08), spreadRadius: 1, blurRadius: 4)],
      ),
      child: Row(
        children: [
          Container(
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(tx.title, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600)),
                const SizedBox(height: 2),
                Text('${tx.dateTime.day}/${tx.dateTime.month}', style: TextStyle(fontSize: 11, color: Colors.grey[600])),
              ],
            ),
          ),
          Text('-RM${tx.amount.toStringAsFixed(2)}', style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600)),
        ],
      ),
    );
  }
  ...
  Stream<List<TransactionModel>> fetchTransactions() {
    ...
    return FirebaseFirestore.instance
        ...
        .limit(5)
        .snapshots()
        .map((snapshot) => snapshot.docs.map((doc) => TransactionModel.fromFirestore(doc.data())).toList());
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Dashboard', style: TextStyle(color: Colors.black, fontSize: 18, fontWeight: FontWeight.w600)),
        centerTitle: true,
        actions: [
          IconButton(icon: const Icon(Icons.notifications_outlined, color: Colors.black), onPressed: () {}),
          Padding(
            padding: const EdgeInsets.only(right: 12.0),
            child: CircleAvatar(backgroundColor: Colors.grey[300], radius: 16, child: const Icon(Icons.person, size: 18, color: Colors.white)),
          ),
        ],
      ),
        ...
        child: Column(
          ...
          children: [
            ...
            StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
              stream: FirebaseFirestore.instance.collection('users').doc(user!.uid).collection('budget').doc('main').snapshots(),
              builder: (context, snapshot) {
                if (!snapshot.hasData) return const CircularProgressIndicator();
                final data = snapshot.data!.data();
                if (data == null) return const Center(child: Text("No budget set"));
                ...
                return FutureBuilder<QuerySnapshot>(
                  ...
                  builder: (context, spendSnapshot) {
                    ...
                    // Budget card color
                    Color cardColor = progress < 0.8 ? Colors.blue.shade50 : progress < 1.0 ? Colors.orange.shade100 : Colors.red.shade200;
                    Color textColor = progress < 0.8 ? Colors.black : Colors.black87;
                    ...
                    return Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text('RM${spending.toStringAsFixed(2)}', style: TextStyle(fontSize: 36, fontWeight: FontWeight.bold, color: textColor)),
                          const SizedBox(height: 12),
                          ClipRRect(
            ...
            // Budget Summary with dynamic data
            StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
              stream: FirebaseFirestore.instance.collection('users').doc(user!.uid).collection('budget').doc('main').snapshots(),
              builder: (context, snapshot) {
                if (!snapshot.hasData) return const CircularProgressIndicator();
                            ...
                            children: [
                              Expanded(
                                child: _buildSummaryItem(
                                  ...
                                  'Budget Left',
                                  'RM${budgetLeft.toStringAsFixed(2)}',
                                ),
                              ),
                              const SizedBox(width: 12),
                              Expanded(
                                child: _buildSummaryItem(
                                  Icons.trending_up,
                                  'Overspent',
                                  'RM${overspent.toStringAsFixed(2)}',
                                ),
                              ),
                            ],
                          ),
                            ...
                            children: [
                              ...
                              Expanded(
                                child: _buildSummaryItem(
                                  ...
                                  'Expected Spending',
                                  'RM${expectedSpending.toStringAsFixed(2)}',
                                ),
                              ),
                            ],
                          ),
                          ...
                          BarChartData(
                            ...
                            titlesData: FlTitlesData(
                                  ...
                                  getTitlesWidget: (value, meta) {
                                    if (value.toInt() >= 0 && value.toInt() < categories.length) {
                                      return Padding(
                                        ...
                                        child: Text(categories[value.toInt()], style: const TextStyle(fontSize: 11)),
                                      );
                                    }
                                    return const Text('');
                                  },
                              ...
                              leftTitles: AxisTitles(
                                sideTitles: SideTitles(showTitles: true, reservedSize: 40, interval: 100),
                              ),
